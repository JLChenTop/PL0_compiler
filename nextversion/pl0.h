...
void showass(){
	...
	for(int i=0;i<maxqnum;i++){
		if(assembly[i]){
			...
			if(opcode[assembly[i]->op]=="pro"){//该条指令把 n3赋给assembly[i]->r1
				//  类似于赋值语句 
				cout.width(10),cout<<assembly[i]->n3;
				cout.width(10),cout<<"-";
				cout.width(10),cout<<assembly[i]->r1->wnode->v;
			}else if(opcode[assembly[i]->op]=="call"){
				//  返回指令的地址在   assembly[i]->n3；
				//	assembly[n3]->n3=i+1;
					if(assembly[i]->n3==-1){//递归调用
					 
					}else{
					assembly[assembly[i]->n3]->n3=i+1;
				//	cout<<"\nret:...."<<assembly[assembly[i]->n3]->n3<<endl;
					cout.width(10),cout<<"-";
					cout.width(10),cout<<"-";
					cout.width(10),cout<<assembly[i]->r1->wnode->v;
					} 
...
//操作数3 	
			...
			}else if(assembly[i]->r3){//非跳转指令   目的操作数应该是一个待操作的数 
			...
			//	else 
				if(opcode[assembly[i]->op]==":=")	//赋值语句，操作数是一个变量,节点是一个叶子节点 
					...
					else cout<<"-"<<assembly[i]->r3->nodeseq;//临时地址空间 
